      ...
      <template slot-scope="{ failed, errors, classes, }">
        <div class="col-md-4 form-checkbox" v-for="option in options" :key="option.id">
          <label class="control-label">
            <input type="checkbox" :id="option.form_identifier" v-model="selectedValue" :value="option" :name="getUniqueName(option.form_identifier)" @change="onChange(selectedValue, label)" :class="classes">
          {{option.label}}</label>
        </div>
