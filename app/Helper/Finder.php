...
class Finder {
    ...
    public static function group($id, $custom = null) {
        if ($custom == null) {
            $group = Permission::whereId($id)->select(['name']);
        } elseif (isset($custom)) {
            $group = Permission::whereId($id)->select($custom);
        }
    ...
    public static function status($id, $custom = null) {
        if ($custom == null) {
            $status = Ticket_Status::where('ticket_status.id', $id)
                    ->join('ticket_status_type', 'ticket_status.purpose_of_status', '=', 'ticket_status_type.id')
                    ->select('ticket_status.*', 'ticket_status_type.name as purpose')
                    ->first();
        } elseif (isset($custom)) {
            $status = Ticket_Status::where('ticket_status.id', $id)
                    ->join('ticket_status_type', 'ticket_status.purpose_of_status', '=', 'ticket_status_type.id')
     ...
     * purpose_of_status 5 means approval and  purpose_of_status 8 means merged
     * @return type array
     */
    public static function getCustomedStatus() {
        return Ticket_Status::select('id', 'icon', 'icon_color', 'name','purpose_of_status')->whereNotIn('purpose_of_status',[5,8])->orderBy('order')->get();
    }
    ...
    public static function defaultStatus($id) {
        $status = Ticket_Status::where('purpose_of_status', $id)->where('default', 1)->first();
        return $status;
    }


    public static function statusApproval() {
        $approval_status = Ticket_Status::where('id', '=', '6')->first();
    ...
    public static function getClientStatus() {
        $status = Ticket_Status::where('purpose_of_status', '!=', 3)->where('purpose_of_status', '!=', 4)->where('id', '!=', 6)->get();
        return $status;
    }

    /**
     * This function is used to get status which is set as default in ticket settings
     * @return int (id of the default status)
     */
    public static function getTicketDefaultStatus()
    {
        return \DB::table('settings_ticket')->first()->status;
    }
}
