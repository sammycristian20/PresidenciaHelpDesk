...
class Handler extends ExceptionHandler
{
    ...
    protected function renderExceptionBasedOnDebugMode($request, Exception $exception)
    {
        ...
        return (config('app.debug') === true) || $exception instanceof MaintenanceModeException ? parent::render($request, $exception) : redirect('server-error');
    }
    ...
    protected function reportDatabaseConnectionFailed(Exception $exception)
    {
        if (get_class($exception)=="Illuminate\Database\QueryException") {
             ...
             */
            if(in_array($exception->getCode(), [1045, 1049, 2002, 2006])) {
                echo "<center><h1>Database connection failed</h1>
                <p style=\"color:red\">{$exception->getMessage()}</p>
                <p><a href=\"mailto:support@ladybirdweb.com\">Report this event to developers</a></p></center>";
    ...
    /**
     * Handles exceptions which occurs during installation process when faveo is not installed.
     * @internal if .env file doesn't exist and some exeception comes, it redirects to probe.php page
     *            Laravel needs application to ready for it to work. Our redirection gets handle
     *            in a middleware but middleware will not be available until application is ready.
     *            Redirecting to probe.php page so that user can see if he missed any dependency to be installed
