...
class TicketController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('jwt.auth', ["only" => "reply"]);
    }
     ...
     * @param String $poster Database field poster in ticket_thread table
     *
     * @return type bool
     */
    public function reply($thread, $request, $poster, $attachments = '', $inlines = '')
    {
        try {
            ...
            $form_data = $request->except('reply_content', 'ticket_id', 'media_attachment', 'inline');
            \Event::dispatch(new \App\Events\ClientTicketFormPost($form_data, $emailadd, $source));

            $reply_content       = $request->input('reply_content');
            $thread->ticket_id   = $request->input('ticket_id');
            $thread->poster      = $poster;
            ...
            if (!($tickets->assigned_to) && Auth::user()->role != 'user') {
            ...
            if (!empty($attachments) || !empty($inlines)) {
                $storage          = new CoreTicketController;
                $check_attachment = $storage->saveReplyAttachment($thread, $attachments, $inlines);
            }
            ...
            try {
                $re = $this->PhpMailController->sendmail($from = $this->PhpMailController->mailfrom('0', $tickets->dept_id), $to = ['name' => $user_name, 'email' => $email], $message = ['subject' => $eventthread->title, 'scenario' => 'ticket-reply', 'body' => $thread->body], $template_variables = ['agent_sign' => Auth::user()->agent_sign, 'ticket_number' => $tickets->ticket_number, 'ticket_link' => url('thread', $tickets->id)]);
            } catch (\Exception $e) {
...
//            $thread = new Ticket_Thread();
            ...
            try {
                $this->PhpMailController->sendmail($from = $this->PhpMailController->mailfrom('0', $ticket->dept_id), $to = ['name' => $agent, 'email' => $agent_email, 'role' => $user->role, 'preferred_language' => $user->user_language], $message = ['subject' => $ticket_subject . '[#' . $ticket_number . ']', 'scenario' => 'assign-ticket'], $template_variables = [
                    'agent_name'    => $agent,
                    'agent_email'   => $user->email,
        ...
        try {
            ...
            foreach ($attach as $file) {
                if (is_object($file)) {
                    ...
                    $ta->create(['thread_id' => $thread, 'name' => $file->getFileName(), 'size' => $file->getSize(), 'type' => $file->getMimeType(), 'file' => $file, 'poster' => 'ATTACHMENT']);
    ...
    public function getCollaboratorForTicket()
    {
        try {
            ...
            $ticket_collaborator = \DB::table('users')
                ...
                ->select('users.email', 'users.user_name', 'users.first_name', 'users.last_name')
                ->get();
            if (count($ticket_collaborator) > 0) {
                foreach ($ticket_collaborator as $key => $collaborator) {
                    ...
                    $collab[$key]['first_name'] = $collaborator->first_name;
                    $collab[$key]['last_name']  = $collaborator->last_name;
