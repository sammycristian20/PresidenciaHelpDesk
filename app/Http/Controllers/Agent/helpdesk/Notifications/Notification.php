...
class Notification extends Controller {
    ...
    public function appNotification($userid=null) {
        $userid = Auth::user()->id;
        $notification = \App\Model\helpdesk\Notification\Notification::with(['requester'=>function($query){
            return $query->select('first_name as changed_by_first_name'
                        , 'last_name as changed_by_last_name'
                        , 'user_name as changed_by_user_name'
                        , 'profile_pic','email','id'
                    );
        }])
    ...
    public function notificationSeen($userid=null, Request $request) {
        $userid = Auth::user()->id;
    ...
    public function notificationUnSeenCount($userid=null) {
        $userid = Auth::user()->id;
        $notification = \App\Model\helpdesk\Notification\Notification::
                whereRaw("find_in_set('$userid',notifications.to)")
                ->select(
                        \DB::raw("find_in_set('$userid',notifications.seen) as seen")
                        , \DB::raw("count(find_in_set('$userid',notifications.seen)) as seen_count")
                )
                ->groupBy('seen')
                ->get()
                ->whereIn('seen', ['0',0])
    ...
    public function notificationUpdateSeenAll($userid=null) {
        try {
            $userid = Auth::user()->id;
    ...
    public function isBrowserMode()
    {
        $check = false;
        if ($this->alert) {
            if($this->alert->getValue('browser_notification_status') == 1) {
                $check = true;
            }
        }
        return $check;
