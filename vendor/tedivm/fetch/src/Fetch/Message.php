...
class Message
{
    ...
    protected function loadMessage()
    {
        ...
        $this->subject = isset($messageOverview->subject) ? $messageOverview->subject
                    : null;
        ...
        $this->from    = isset($headers->from) ? $this->processAddressObject($headers->from)
                    : array('');
        $this->replyTo = isset($headers->reply_to) ? $this->processAddressObject($headers->reply_to)
                    : $this->from;
    ...
    public function getHeaders($forceReload = false)
    {
        if ($forceReload || !isset($this->headers)) {
            ...
            $this->auto_respond = $this->isAutoResponds($rawHeaders);
    ...
    public function getAddresses($type, $asString = false)
    {
        ...
        if (!in_array($type, $addressTypes) || !isset($this->$type) || count($this->$type)
                < 1) return false;

        if (!$asString) {
            if ($type == 'from') return $this->from[0];
            elseif ($type == 'sender') return $this->sender[0];

            return $this->$type;
        } else {
            $outputString = '';
            foreach ($this->$type as $address) {
                if (isset($set)) $outputString .= ', ';
                if (!isset($set)) $set = true;
    ...
    protected function processStructure($structure, $partIdentifier = null)
    {
        ...
        if ((isset($parameters['name']) || isset($parameters['filename'])) || (isset($structure->subtype)
        ...
        elseif ($structure->type == 0 || $structure->type == 1) {
            ...
            if (strtolower($structure->subtype) === 'plain' || ($structure->type
                    == 1 && strtolower($structure->subtype) !== 'alternative')) {
    ...
    public function getAttachments($filename = null)
    {
        ...
        if (!isset($filename)) return $this->attachments;
    ...
    public function isAutoResponds($rawHeaders)
    {
        $array = ['Auto-Submitted', 'X-Autoreply'];
        return str_contains($rawHeaders, $array);
    }
}
